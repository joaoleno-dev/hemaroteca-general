...
class VeiculosController extends AppController {
	...
	function view($id = null) {
		if (!$id) {
			$this->Session->setFlash(__('Invalid veiculo', true),'flash_bad');
	...
	function add() {
		if (!empty($this->data)) {
			...
			if ($this->Veiculo->save($this->data)) {
				$this->Session->setFlash(__('The veiculo has been saved', true),'flash_good');
				$this->redirect(array('action' => 'index'));
			} else {
				$this->Session->setFlash(__('The veiculo could not be saved. Please, try again.', true),'flash_bad');
			}
		}
	...
	function delete($id = null) {
		if (!$id) {
			$this->Session->setFlash(__('Invalid id for veiculo', true),'flash_bad');
			$this->redirect(array('action'=>'index'));
		}
		if ($this->Veiculo->delete($id)) {
			$this->Session->setFlash(__('Veiculo deleted', true),'flash_good');
			$this->redirect(array('action'=>'index'));
		}
		$this->Session->setFlash(__('Veiculo was not deleted', true),'flash_bad');
		$this->redirect(array('action' => 'index'));
	}
}
